    ...
    if (allZillaWeather.isEmpty) {
      ...
      addFirstZilla()
          .whenComplete(() => Future.delayed(Duration(seconds: 5), () async {
                await addSecondZilla()
                    .whenComplete(() => {progressIndicator.value = 0.3});
                sleep(Duration(seconds: 3));

                await addThirdZilla()
                    .whenComplete(() => {progressIndicator.value = 0.5});

                sleep(Duration(seconds: 3));
                await addfourthZilla()
                    .whenComplete(() => {progressIndicator.value = 0.7});

                sleep(Duration(seconds: 3));
                await addFifthZilla()
                    .whenComplete(() => {progressIndicator.value = 1.0});
